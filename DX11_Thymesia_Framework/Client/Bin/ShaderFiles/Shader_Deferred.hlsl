...
PS_OUT_LIGHT PS_MAIN_LIGHT_POINT(PS_IN In)
{
   ...
   // vLightFlagDesc *= g_vLightFlag;
    ...
    fAtt *= g_fLightIntensity * fIntensity;
    vector vLook = normalize(g_vCamPosition - vWorldPos);
...
PS_OUT PS_MAIN_BLEND(PS_IN In)
{
   ...
   // vector vLightFlagDesc = g_LightFlagTexture.Sample(DefaultSampler, In.vTexUV);
    ...
    if (vAmbientDesc.a > 0.f)
    {
        ...
        float3 vMetalic = lerp(vDiffuse.xyz*0.04f, vDiffuse.xyz, fMetalness);
      ...
      //irradiance
        ...
        vector kD = vector(lerp(1.0 - F, 0.0, fMetalness), 0.f);
     
        vector vIrradiance = g_IrradianceTexture.Sample(ClampSampler, vNormal);
        vector vIrradianceDiffuse = vIrradiance * vDiffuse * kD;
          
        vector vReflect = reflect(-normalize(vLook), vNormal);
        
        float3 prefilteredColor = g_PreFilterTexture.SampleLevel(ClampSampler, vReflect.xyz, (fRoughness * 10.f)).rgb;
        float2 envBRDF = g_BRDFTexture.Sample(ClampSampler, float2(NdotV, fRoughness)).rg;
        vector specular = vector(prefilteredColor * (F * envBRDF.x + envBRDF.y), 0.f) * fOcclusion * (1.f - fRoughness);
 
        specular = pow(specular, 2.2f);
        vIrradianceDiffuse = pow(vIrradianceDiffuse, 2.2f);
        
        specular.a = 0.f;
        vIrradianceDiffuse.a = 0.f;
        
        Out.vColor = vAmbientDesc + vSpecular /* * (1 - bIsInShadow)*/ + vIrradianceDiffuse + specular;
