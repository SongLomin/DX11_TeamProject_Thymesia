   ...
   // vector vLightFlagDesc = g_LightFlagTexture.Sample(DefaultSampler, In.vTexUV);
    ...
    if (vAmbientDesc.a > 0.f)
    {
      ...
      //irradiance
        ...
        float3 prefilteredColor = g_PreFilterTexture.SampleLevel(ClampSampler, vReflect.xyz, (int)fRoughness * 10.f).rgb;
        float2 envBRDF = g_BRDFTexture.Sample(ClampSampler, float2(NdotV, fRoughness)).rg;
        vector specular = vector(prefilteredColor * (F * envBRDF.x + envBRDF.y), 0.f) * fOcclusion  *fMetalness;
