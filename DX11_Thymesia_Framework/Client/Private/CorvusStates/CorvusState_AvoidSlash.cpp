...
void CCorvusState_AVoidSalsh::Start()
{
	...
	m_iAnimIndex = m_pModelCom.lock()->Get_IndexFromAnimName("SK_C_Corvus.ao|Corvus_SD_AvoidAttack_D");
...
void CCorvusState_AVoidSalsh::OnStateStart(const _float& In_fAnimationBlendTime)
{
	...
	if (Get_OwnerCharacter().lock()->Get_PreState().lock() == Get_Owner().lock()->Get_Component<CCorvusState_AVoidSalsh>().lock())
	{
