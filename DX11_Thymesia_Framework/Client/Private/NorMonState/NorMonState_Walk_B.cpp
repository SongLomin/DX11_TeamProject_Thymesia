...
void CNorMonState_Walk_B::Start()
{
	...
	{
		m_iAnimIndex = m_pModelCom.lock()->Get_IndexFromAnimName("Armature|Armature|LV1Villager_M_WalkB|BaseLayer");
	}

	m_pModelCom.lock()->CallBack_AnimationEnd += bind(&CNorMonState_Walk_B::Call_AnimationEnd, this);
}
...
void CNorMonState_Walk_B::Call_AnimationEnd()
{
	if (!Get_Enable())
		return;
...
void CNorMonState_Walk_B::OnDestroy()
{
	m_pModelCom.lock()->CallBack_AnimationEnd -= bind(&CNorMonState_Walk_B::Call_AnimationEnd, this);
}
