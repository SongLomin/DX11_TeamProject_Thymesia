...
void CCamera_Target::Look_At_Target(_float fTimeDelta)//타겟 고정
{
	...
	//matTargetMonsterWorld.r[0] = XMVector3Normalize(matTargetMonsterWorld.r[0]);
	//matTargetMonsterWorld.r[1] = XMVector3Normalize(matTargetMonsterWorld.r[1]);
	//matTargetMonsterWorld.r[2] = XMVector3Normalize(matTargetMonsterWorld.r[2]);
	...
	_vector vTargetPos = m_pTargetMonsterTransformCom.lock()->Get_Position();//matTargetMonsterWorld.r[3];
