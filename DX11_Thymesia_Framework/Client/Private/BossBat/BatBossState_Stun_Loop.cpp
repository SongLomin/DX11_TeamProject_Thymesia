...
void CBatBossState_Stun_Loop::Start()
{
	...
	m_iAnimIndex = m_pModelCom.lock()->Get_IndexFromAnimName("SK_C_BossBat_NEW_V1.ao|BossBat_StunLoop");

	m_pModelCom.lock()->CallBack_AnimationEnd += bind(&CBatBossState_Stun_Loop::Call_AnimationEnd, this);
}
...
void CBatBossState_Stun_Loop::Call_AnimationEnd()
{
	if (!Get_Enable())
		return;
...
void CBatBossState_Stun_Loop::OnDestroy()
{
	m_pModelCom.lock()->CallBack_AnimationEnd -= bind(&CBatBossState_Stun_Loop::Call_AnimationEnd, this);
}
